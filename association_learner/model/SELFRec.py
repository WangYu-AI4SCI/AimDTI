...
class SELFRec(object):
    def __init__(self, config):
        ...
        self.validation_data = FileIO.load_data_set(config['validation.set'], config['model']['type'])
    ...
    def execute(self):
        ...
        exec(import_str)  # exec("from model.graph.LightGCN import LightGCN")
        recommender = f"{self.config['model']['name']}(self.config,self.training_data,self.validation_data,self.test_data,**self.kwargs)"
        eval(recommender).execute() # eval("LightGCN(self.config, self.training_data, self.test_data, **self.kwargs)").execute()
